...
- **News** reads official Omarchy and Omacom Foundation announcements from the RSS feed in a two-pane desktop reader. It refreshes every 15 minutes by default, and the bar icon marks unread announcements. Open the feed manager from the reader header or press `F` to enable curated developer, technology, Linux and AI sources, or add up to ten custom public HTTPS RSS feeds. Custom subscriptions are verified before they are saved and can be renamed, reordered or removed without editing configuration. The source rail uses the active theme's palette to distinguish feed categories; use `[` and `]` to switch between sources. When a publisher supplies only an excerpt, choose **Open Original** or press `O` to continue to the full article.
